...
### 0. **输入验证**
- 若{name}同志的自我评价内容（{text}）存在以下问题，请直接打分为0，跳过分析并评价：
  - 为空或仅包含无意义话语
  - 明显胡言乱语、随机字母或不完整语句
  - 含有攻击性、低俗、不适当内容（如脏话、人身攻击等）
  - 过度空泛、毫无实质内容（如“我很好”“没什么可说的”）
  - 过长但无结构，明显是无意义堆砌（如重复的短语、过度冗长但无重点）
...
### 3. **评分规则（60-100）**
- **基础评分标准**（不考虑标签权重时）：  
  - **60-70**：该同志党员意识和党性修养有待加强 或者 内容与标签关联较弱
  - **71-85**：该同志需进一步提升党性修养和实践经验 或者 内容与标签基本匹配但缺乏细节
  - **86-100**：该同志充分展现党员先锋模范作用 或者 内容详实且与标签高度契合
- **额外加权规则**：  
  - **金色传说标签**：+10 至 +15  
  - **紫色稀有标签**：+5 至 +10  
  - **绿色超凡标签**：+1 至 +5  
...
if __name__ == "__main__":
    ...
    name3 = "王五"
    tag3 = [('领导能力', '紫色稀有'),('决策能力', '紫色稀有')]
    text3="aaaaa"
    evaluate_with_openai(name3,tag3,text3)
